...
def index():
    if request.method == 'POST':
        ...
        return render_template('index.html', result=result, text_to_check=text_to_check)
    
    return render_template('index.html', result=None, text_to_check=None)
...
def check_plagiarism(text):
    ...
    burp0_data = {"is_free": "false", "plagchecker_locale": "en", "product_paper_type": "1", "title": '', "text": text}
    ...
    try:
        ...
        # Check if the 'words_count' key is present in the result dictionary
        if 'words_count' in result:
            ...
            return {'error': "'words_count' not found in the API response."}

    except requests.exceptions.RequestException as e:
        return {'error': f'Error making the request: {e}'}
